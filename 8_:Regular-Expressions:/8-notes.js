...
// The Search Method
// The indexOf method on strings cannot be called with a regular expression. But there is another method, "search", which does expect
// a regular expression. Like indexOf, it returns the first index on which the expression was found, or -1 when it wasn't found.
...
// indexOf), which can often be useful.


// The LastIndex Property
// The exec method similarly does not provide a convenient way to start searching from a given position in the string. But it does
// provide an inconvenient way.

// Regular expression objects have properties. One such property is source, which contains the string that expression was created
// from. Another property is lastIndex, which controls, in some limited circumstances, where the next match will start.

// Those circumstances are that the regular expression must have the global (g) option enabled, and the match must happen through
// the exec method. Again, a more sane solution would have been to just allow an extra argument to be passed to exec, but sanity is
// not a defining characteristic of JavaScript's regular expression interface.
var pattern = /y/g;
pattern.lastIndex = 3;
...
// If no match was found, lastIndex is set back to zero, which is also the value it has in a newly constructed regular expression
// object.

// When using a global regular expression value for multiple exec calls, these automatic updates to the lastIndex property can cause
// problems. Your regular expression might be accidentally starting at an index that was left over from a previous call.
var digit = /\d/g;
...
// Another interesting effect of the global option is that it changes the way the match method on strings works. When called with a
// global expression, instead of returning an array similar to that returned by exec, match will find all matches of the pattern in
// the string and return an array containing the matched strings.
console.log("Banana".match(/an/g));
// -> ["an", "an"]

// So be cautious with global expressions. The cases where they are necessary -- calls to replace and places where you want to
// explicitly use lastIndex -- are typically the only places where you want to use them.
